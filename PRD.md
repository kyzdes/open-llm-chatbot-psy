...
- Эмпатичный AI-психолог на базе LLM (бесплатные модели через OpenRouter)
- Полная персистентная история диалогов (SQLite)
- Скользящее окно контекста (~100K токенов)
...
- Мгновенная проверка ключевых слов (рус/англ) через предкомпилированный regex
- Автоматическое предоставление контактов горячих линий
- Логирование кризисных событий в БД
...
- HTML-форматирование сообщений — надёжный рендеринг жирного, курсива, кода
...
- **Конфигурация**: дефолтная модель задаётся через `DEFAULT_MODEL` в переменных окружения
- **Rate Limiting**: Token Bucket (3 burst, 1 сообщение / 10 сек)
- **Таймаут LLM**: 120 секунд с retry (3 попытки, exponential backoff для 429/5xx)
- **Fallback-модель**: при отказе основной модели автоматический переход на `openrouter/free`
