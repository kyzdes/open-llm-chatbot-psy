...
[Handler: therapy]
  ...
  5. LLM запрос → OpenRouter (retry 3x при 429/timeout)
...
- **Фильтрация**: бесплатные (`pricing.prompt == "0"`) + поддержка `system_prompt`
...
Модель по умолчанию: `stepfun/step-3.5-flash:free` (настраивается в `config.py`).
